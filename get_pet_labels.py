...
def get_pet_labels(image_dir):
    ...
    for idx in range(0, len(in_files), 1):
       ...
       if in_files[idx][0] != ".":
           ...
           pet_label = (' '.join([str(x) for x in in_files[idx].split('_')[:-1]])).lower()
    ...
    #    filename = file
    #    label = (' '.join([str(x) for x in filename.split('_')[:-1]])).lower() # 'boston terrier'
    #    result_dict = {filename:label for filename,label in filename_list}
    #return result_dict
